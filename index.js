...
function summarise(days) {
  return {
    length: days.length,
    precipIntensityMax: days.reduce(
      (currentMax, day) => Math.max(day.precipIntensityMax, currentMax),
      0
    ),
    precipIntensity: days.reduce(
      (currentTotal, day) => currentTotal + day.precipIntensity,
      0
    ),
    precipProbabilityMax: days.reduce(
      (currentMax, day) => Math.max(day.precipProbability, currentMax),
      0
    ),
    apparentTemperatureLowAvg:
      days.reduce(
        (currentTotal, day) => currentTotal + day.apparentTemperatureLow,
        0
      ) / days.length,
    apparentTemperatureHighAvg:
      days.reduce(
